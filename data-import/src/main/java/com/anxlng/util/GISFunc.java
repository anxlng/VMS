...
public class GISFunc {
    ...
    /**
     * 度分格式转成度格式 如：2139.2500N， 表示北纬 21度，39.2500分
     * @param dm
     * @return
     */
    public static double dmToDeg(String dm) {
        int len = dm.length();
        char f = dm.charAt(len - 1);
        int index = dm.indexOf(".");
        double min = Double.parseDouble(dm.substring(index - 2, len - 1));
        double deg = Double.parseDouble(dm.substring(0, index - 2)) + min / 60D;
